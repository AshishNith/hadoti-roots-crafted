    ...
    <>
      <header
        className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${
          scrolled
            ? "bg-[color:var(--cream)]/95 backdrop-blur shadow-[0_1px_0_rgba(0,0,0,0.06)] py-3"
            : "bg-transparent py-6"
        }`}
      >
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 flex items-center justify-between gap-6">
          <Link
            ...
            style={{ color: scrolled ? "var(--earth)" : "var(--earth)" }}
          >
            Hadoti Farms
          </Link>
          ...
          <div className="flex items-center gap-5">
            <button aria-label="Search" className="hover:text-[color:var(--earth)] transition-colors">
              <Search size={18} />
            </button>
            <Link to="/account" aria-label="Account" className="hidden sm:inline-flex hover:text-[color:var(--earth)] transition-colors">
              <User size={18} />
            </Link>
            <Link to="/cart" aria-label="Cart" className="relative hover:text-[color:var(--earth)] transition-colors">
              <ShoppingBag size={18} />
              {count > 0 && (
                <span className="absolute -top-2 -right-2 bg-[color:var(--earth)] text-white text-[10px] font-mono w-5 h-5 rounded-full flex items-center justify-center">
                  {count}
                </span>
              )}
            </Link>
